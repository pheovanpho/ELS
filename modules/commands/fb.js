...
try { 
  if(args[0] == 'audio'){
        api.sendMessage(`[⚜️]→ Đang xử lí yêu cầu!!!`, event.threadID, (err, info) =>
    setTimeout(() => {
...
return api.sendMessage({body : `[⚜️]→ Loaded success`, 
    attachment: fs.createReadStream(path)}, event.threadID, () => fs.unlinkSync(path),event.messageID);
    }; 
  }catch {return api.sendMessage(`[⚜️]→ Không thể xử lý yêu cầu`,event.threadID,event.messageID)}
    try { 
      if(args[0] == 'video'){
            api.sendMessage(`[⚜️]→ Đang xử lí yêu cầu!!!`, event.threadID, (err, info) =>
    setTimeout(() => {
