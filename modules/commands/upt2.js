...
let l1 = await loadImage(pathAva);
  ...
  ctx.drawImage(l1, 100, -290, 1600, 1700);
    ...
    ctx.fillText("JRT X TRACY", 1820, 550);
    ctx.beginPath();
  ////////////////////////////////////////
   registerFont(__dirname + `/tad/UTM-Avo.ttf`, {
    ...
    ctx.fillText(`${z_1} : ${x_1} : ${y_1} `, 1890, 640);
    ctx.restore();
    ctx.save();
registerFont(__dirname + `/tad/CaviarDreams.ttf`, {
    ...
    /*ctx.textAlign = "start";
    ctx.font = "45px time";
    ctx.fillText("Never gonna", 240, 590)
    ctx.fillText("give you", 240, 670)
    ctx.fillText("up", 405, 750)
    ctx.restore();
    ctx.save();
    ctx.beginPath();*/
  ...
  return api.sendMessage({
    body: `[⚜️]=== 『 𝑼𝑷𝑻𝑰𝑴𝑬 𝑹𝑶𝑩𝑶𝑻 』 ===[⚜️]\n◆━━━━━━━━━━━━━━━━◆\n\n[⚜️]➜ 𝗕𝗼𝘁 𝗢𝗻𝗹𝗶𝗻𝗲 𝗧𝗿𝗼𝗻𝗴 ${hours} 𝗚𝗶𝗼̛̀ ${minutes} 𝗣𝗵𝘂́𝘁 ${seconds} 𝗚𝗶𝗮̂𝘆.\n\n[⚜️]➜ 𝗡𝗴𝘂̛𝗼̛̀𝗶 𝗗𝘂̀𝗻𝗴: ${global.data.allUserID.length}\n[⚜️]➜ 𝗡𝗵𝗼́𝗺: ${global.data.allThreadID.length}\n[⚜️]➜ 𝗖𝗽𝘂: ${pidusage.cpu.toFixed(1)}%\n[⚜️]➜ 𝗥𝗮𝗺: ${byte2mb(pidusage.memory)}\n[⚜️]➜ 𝗣𝗶𝗻𝗴: ${Date.now() - timeStart}ms\n[⚜️]➜ 𝗜𝗗: ${id}`,
