...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   if (args.length == 0) return api.sendMessage(
     `=== 『 CHECK 』 ===\n` + 
     `--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} ndfb => Lọc thành viên bị bay acc khỏi nhóm` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} del => Lọc thành viên khỏi nhóm theo số tin nhắn` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} onl => Xem thời gian hoạt động bot onl` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} tt => Check độ tương tác của bạn` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} box => Lọc nhóm dưới 4 thành viên` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} all => Check tất cả độ tương tác các thành viên trong box` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} covid => Xem thông tin covid` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} mayman => Xem Tỉ lệ % may mắn của bạn?` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} age => Check age (tuổi) của bạn` +
     `\n\n━━━━━━━━━━━━━━━\n[⚜️]=== 『 𝐁𝐎𝐓 ELS  』 ===[⚜️]\n\n===「${timeNow}」===`, event.threadID, event.messageID);
    var arr = [];
    var mention = Object.keys(event.mentions);
    const data = await api.getThreadInfo(event.threadID);
    if (args[0] == "ndfb") {// kick người dùng fb
    if (permssion < 1) return api.sendMessage("[DONATE]→ Momo/Mbbank: 0836645838. Xin cám ơn ạ!! ❤️", threadID, messageID);
    ...
    if (!find) return api.sendMessage(`[⚜️] => Bạn và bot cần là quản trị viên!`,event.threadID);
      ...
      if (permssion != 3) return api.sendMessage("[DONATE]→ Momo/Mbbank: 0836645838. Xin cám ơn ạ!! ❤️", threadID, messageID);
    const find = data.adminIDs.find(el => el.id == event.senderID && api.getCurrentUserID());
    if (!find) return api.sendMessage(`[⚜️] => Bạn và bot cần là quản trị viên!`,event.threadID);
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 100009355185480) return api.sendMessage(`[DONATE]→ Momo/Mbbank: 0836645838. Xin cám ơn ạ!! ❤️`, event.threadID, event.messageID)
           ...
           return api.sendMessage(`[⚜️] => Đang tiến hành lọc những nhóm không tên và dưới 4 thành viên.`,threadID)
    }
else if (args[0] == "image") {
  const axios = global.nodemodule['axios'];  
var linkanh = event.messageReply.attachments[0].url || args.join(" ");
	if(!linkanh) return api.sendMessage('[⚜️]→ Vui lòng reply hoặc nhập link 1 hình ảnh!!!', event.threadID, event.messageID)
...
var img = res.data.uploaded.image;
	...
	let seconds = Math.floor(time % 60);
      const timeStart = Date.now();
       return api.sendMessage('[⚜️] => Đang kiểm tra kết nối vui lòng chờ⏳... !████████████ 99%', event.threadID, (err, info) => {
    setTimeout(() => {
      api.sendMessage(`[⚜️] => Ping: ${(Date.now() - timeStart)}ms \n[⚜️] => TimeOn: ${hours} Giờ ${minutes} Phút ${seconds} Giây`, event.threadID, event.messageID);
    }, 200);
  }, event.messageID);
...
var callback = () => api.sendMessage({body:`[⚜️] Tỉ lệ may mắn của bạn là ${tile}%`, attachment: fs.createReadStream(__dirname + "/cache/tile.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/tile.jpg")); 
  ...
  var input = args[0];
  if (!input) return api.sendMessage("[⚜️]→ Sai định dạng", event.threadID);
  var content = input.split("-");
  var dayout = parseInt(content[0]);
  if (!dayout || isNaN(dayout) || dayout > 31 || dayout < 1) { return api.sendMessage("[⚜️]→ Ngày sinh không hợp lệ!", event.threadID)}
  var monthout = parseInt(content[1]);
  if (!monthout || isNaN(monthout) || monthout > 12 || monthout < 1) { return api.sendMessage("[⚜️]→ Tháng sinh không hợp lệ!", event.threadID)}
  var yearout = parseInt(content[2]);
  if (!yearout || isNaN(yearout) || yearout > yearin || yearout < 1) { return api.sendMessage("[⚜️]→ Năm sinh không hợp lệ!", event.threadID)}
  var tuoi = yearin - yearout
  var msg = `[⚜️]→ Năm nay bạn ${tuoi} tuổi.`
  if (monthout > monthin) {var tuoi = tuoi - 1; var aftermonth = monthout - monthin; var msg = `[⚜️]→ Năm nay bạn ${tuoi} tuổi. Còn ${aftermonth} tháng nữa là bạn sẽ tròn ${tuoi + 1} tuổi.`};
  if (monthin == monthout && dayin < dayout) {var tuoi = tuoi - 1; var afterday = dayout - dayin; var msg = `[⚜️]→ Năm nay bạn ${tuoi} tuổi. Còn ${afterday} ngày nữa là bạn sẽ tròn ${tuoi + 1} tuổi.`};
