...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  if(args[0] == "find" || args[0] == "tìm"){
    const lengthchar = (await axios.get(`https://docs-api.nguyenhaidang.ml/taoanhdep/search?type=id&id=${args[1]}`)).data
    const t = (await axios.get(lengthchar.imgAnime, {
        responseType: "stream"
      })).data;
    var msg = ({
      body: `[⚜️]➜ ID NHÂN VẬT ${args[1]}\n[⚜️]➜ MÀU MẶC ĐỊNH ${lengthchar.colorBg}`,
      attachment: t
    })
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  const Canvas = require('canvas');
    ...
    if(handleReply.step == 1){
      ...
      return api.sendMessage(`[⚜️]➜ Bạn đã chọn char mang số thứ tự là ${event.body}\n[⚜️]➜ Reply tin nhắn này để nhập tên của bạn`, event.threadID, (err, info) => {
    return global.client.handleReply.push({
      step: 2,
      name: this.config.name,
      chartid: event.body,
      ...
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`[⚜️]➜ Bạn đã chọn tên của mình là ${event.body}\n[⚜️]➜ Reply tin nhắn này để nhập màu bạn muốn`, event.threadID, (err, info) => {
    return global.client.handleReply.push({
      step: 3,
      name: this.config.name,
      ...
      if(!color || color == "no" || color == "No"){
        const lengthchar = (await axios.get(`https://docs-api.nguyenhaidang.ml/taoanhdep/search?type=id&id=${id}`)).data
      var color_ = lengthchar.colorBg
    } else {
      var color_ = color
    }
    const lengthcharr = (await axios.get(`https://docs-api.nguyenhaidang.ml/taoanhdep/search?type=id&id=${id}`)).data
let avtAnime = (
